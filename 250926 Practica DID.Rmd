...
# Practice 4: Staggered DID estimation using `Stata`
...
## Dataset description using `Stata`
...
* 
tabstat deuda_r_pc, statistics( min mean median max sd ) by(regions) labelwidth(30) columns(statistics) longstub format(%10.2g)
...
\newpage
# Practice 5: Staggered DID estimation using `R`

+ Here we will be reproducing the same analysis of Practice 4, but using `R`. 

## CSDID estimation using `R` (without covariates)
...
## CSDID estimation using `R` (with covariates)
